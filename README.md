...
<details>
<summary>Prefer not to pipe a script into your shell? Here is the manual way</summary>
...
| 5 | **Cohere** | [dashboard.cohere.com/api-keys](https://dashboard.cohere.com/api-keys) | 20/min · 1,000/**month** (~33/day) | ✅ 0.6–2.7 s | A genuinely different model family, so a useful second opinion when an answer looks wrong |
...
| Pollinations | no key needed | credit-limited | not tested | Keyless emergency fallback, community-hosted. Off by default |
...
- **SambaNova, Hugging Face and Together no longer offer permanent free tiers**,
  only trial credits, so they are deliberately not included.
...
Checked by parsing the command, not pattern-matching, so `sudo rm -rf /home/you`
is caught too.
...
tier that is an instant `413 Request too large`, so the agent cannot run at all.
